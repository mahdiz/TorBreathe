...
namespace TorBricks
{
	public partial class MainForm : Form
	{
        ...
        private const int defaultUserCount = 1024;
        ...
        private void TestForm_Load(object sender, EventArgs e)
        {
            ...
            tbBadCountMax.Value = (int)Math.Pow(2, tbUserCount.Value - 2);
            tbDistCount.Value = defaultDistCount;
            //chPlots.Series.Clear();
            rbMultipleRuns.Checked = true;
            rbGridView.Checked = true;
            cboGridX.Checked = true;
            cboGridY.Checked = true;
            rbLegendLeft.Checked = true;
            tbCorruptCount.Value = 0;
        }
        ...
        private bool OnRoundEnd(int round, int bridgeCount, int blockedCount, int bridgesSoFar)
        {
            ...
            //    blockedCount.ToString(), bridgesSoFar.ToString(), thirstyCount.ToString(),
            //});
        ...
        private void OnRunEnd(int numRound, int bridgesUsed)
        {
			var userCount = Simulator.NodeCount<User>();
			var corruptCount = Simulator.NodeCount<CorruptUser>();
			var thirstyCount = Simulator.GetNodes<User>().Where(u => u.IsThirsty && !(u is CorruptUser)).Count();
			...
			// Add one new row to the grid view
			dgvStats.Rows.Add(new string[] {
                numRound.ToString(), userCount.ToString(), corruptCount.ToString(),
				"N/A", "N/A",
				"N/A", bridgesUsed.ToString(), "N/A",
			});
            Application.DoEvents();
		}
